...
%   spectra.
%
%   The program uses the same code to read both HR-1024 and HR-1024i files.
%   However there appears to be at least one slight difference between
%   these two file formats. The difference occurs in the 'scan settings'
%   header field.
%
...
for i = 1:length(listOfFileNames)
    ...
    if regexp(file, 'HR-1024') % Could be HR-1024 or HR-1024i.
...
% Date and time
% The date and time in SVC GER1500 signature files could be either of the
% following two formats:
%   American: time= 6/28/2010 12:45:04 PM, 12:00:00 AM
%   British:  time= 11/06/2010 12:54:56,11/06/2010 12:54:48
%
% Note that in the American format the date appears only once. It's
% reasonable to assume the target and reference spectra were recorded on
% the same date!
%
% Work out the date and time format by searching for one of the above two patterns.
if ~isempty( regexp(file, '^time=\s*\d+/\d+/\d+ \d+:\d+:\d+ [AP]M,\s*\d+:\d+:\d+ [AP]M', 'once', 'lineanchors') )
    % The date and time are in the American format.
    dateString = regexp(file, '^time=\s*(\d+/\d+/\d+) \d+:\d+:\d+ [AP]M,\s*\d+:\d+:\d+ [AP]M', 'tokens', 'once', 'lineanchors');
    targetTime = regexp(file, '^time=\s*\d+/\d+/\d+ (\d+:\d+:\d+ [AP]M),\s*\d+:\d+:\d+ [AP]M', 'tokens', 'once', 'lineanchors');
    referenceTime = regexp(file, '^time=\s*\d+/\d+/\d+ \d+:\d+:\d+ [AP]M,\s*(\d+:\d+:\d+ [AP]M)', 'tokens', 'once', 'lineanchors');
    % Parse to give datetimes for the target and reference spectra.
    dateVector = datevec(dateString, 'mm/dd/yyyy');
    targetTimeVector = datevec(targetTime, 'HH:MM:SS AM');
    referenceTimeVector = datevec(referenceTime, 'HH:MM:SS AM');
    targetDatetimeVector = [ dateVector(1:3), targetTimeVector(4:6) ];
    referenceDatetimeVector = [ dateVector(1:3), referenceTimeVector(4:6) ];
    T.DateTime = datestr(targetDatetimeVector);
    R.DateTime = datestr(referenceDatetimeVector);
elseif ~isempty( regexp(file, '^time=\s*\d+/\d+/\d+ \d+:\d+:\d+,\s*\d+/\d+/\d+ \d+:\d+:\d+', 'once', 'lineanchors') )
    % The British format
    warning('This function cannot read dates in the British format.')
    T.DateTime = 'Unknown';
    R.DateTime = 'Unknown';
end
...
% Fore optic
opticNames = { 'Standard 4 degree field of view', 'Fiber optic', '8 degree field of view', 'Diffuser', 'Unknown (optic number 5)'};
targetOpticNumber = str2double(regexp(file, '^optic= (\d+)', 'tokens', 'once', 'lineanchors'));
if targetOpticNumber == 0
    warning('Optic number 0 was used for the target spectrum in the file:\n\t%ss\nThe SVC GER1500 does not explain what optic number 0 is. Perhaps it indicates that no optic was attached.', T.Name);
    T.ForeOptic = 'Unknown';
else
...
if referenceOpticNumber == 0
    warning('Optic number 0 was used for the reference spectrum in the file:\n\t%ss\nThe SVC GER1500 does not explain what optic number 0 is. Perhaps it indicates that no optic was attached.', T.Name);
    R.ForeOptic = 'Unknown';
else
...
if isempty(referenceDarkTypeCode)
    R.DarkType = 'unknown';
else 
...
if R.ErrorCode ~= 0 || T.ErrorCode ~= 0
    warning('PostProcessing:SpectrometerReportedError', 'The spectrometer reported an error code whilst recording the spectra named:\n\t%s', T.Name);
end
